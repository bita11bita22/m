"""
پنل مدیریت XRAY — FastAPI + Nginx Stats + Per-User IP Tracker
بهینه شده برای مصرف رم و دیسک کم
"""
...
# ساختار: { "uid": { "ip1": last_seen_time, "ip2": last_seen_time } }
active_ips = {}  
total_unique_users = set()

# ── Xray Core Manager ────────────────────────────────────
def load_data():
    global LINKS, total_unique_users
    try:
...
def save_stats():
    with open(STATS_FILE, "w") as f:
        json.dump({"total_unique": list(total_unique_users), "bytes": stats["bytes"]}, f)
...
async def stats_updater():
    ...
    while True:
        # ۱. خواندن ترافیک از Nginx
        try:
            if os.path.exists(NGINX_LOG):
                # پاکسازی لاگ Nginx اگر از ۱ مگابایت بیشتر شد
            ...
            if os.path.exists(XRAY_LOG):
                ...
                with open(XRAY_LOG, "r") as f:
                    ...
                    for ip, uid in matches:
                        if uid not in active_ips:
                            active_ips[uid] = {}
                        active_ips[uid][ip] = time.time()
                        total_unique_users.add(uid)
                        
                # پاکسازی IPهای غیرفعال (بیش از ۵ دقیقه فعالیت نداشته‌اند)
                now = time.time()
                for uid in list(active_ips.keys()):
                    for ip in list(active_ips[uid].keys()):
                        if now - active_ips[uid][ip] > 300:
                            del active_ips[uid][ip]
                    if not active_ips[uid]:
                        del active_ips[uid]
...
async def api_stats(request: Request, token: Optional[str] = Cookie(None)):
    ...
    total_ips_online = sum(len(ips) for ips in active_ips.values())
    return {
        "total_users": len(LINKS),
        "total_connected": len(total_unique_users),
        "active_uuids": len(active_ips),
        "active_ips": total_ips_online,
        "bytes": stats["bytes"],
        "uptime": uptime_str()
    }
...
async def api_links(request: Request, token: Optional[str] = Cookie(None)):
    ...
    for uid, info in LINKS.items():
        ip_count = len(active_ips.get(uid, {}))
        out.append({
            "uuid": uid, 
            "label": info["label"], 
