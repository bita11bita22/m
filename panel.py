...
async def stats_updater():
    ...
    while True:
        # ۱. خواندن ترافیک از API داخلی Xray (برای Reality و بقیه)
        try:
...
async def api_stats(request: Request, token: Optional[str] = Cookie(None)):
    ...
    return {
        ...
        "active_ips": sum(len(ips) for ips in active_ips.values()), 
        "bytes": stats["bytes"], 
        "uptime": uptime_str()
    }
...
async def api_links(request: Request, token: Optional[str] = Cookie(None)):
    ...
    for uid, info in LINKS.items():
        out.append({
            ...
            "online_ips": len(active_ips.get(uid, {})),
...
@app.get("/health")
async def health(): return {"status": "ok", "connections": sum(len(ips) for ips in active_ips.values())}
