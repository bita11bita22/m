"""
پنل مدیریت XRAY — FastAPI + Nginx + Reality + XHTTP Reality (Single Port Fallback)
"""
...
# پورت داخلی برای fallback کردن XHTTP
...
def sync_xray_config():
    ...
    inbounds = [
        ...
        # 3. Internal XHTTP for Reality Fallback
    ...
    if reality_keys["priv"]:
        # 4. Reality TCP (Single Port with Fallback)
        inbounds.append({
            "port": REALITY_PORT, "listen": "0.0.0.0", "protocol": "vless",
            "settings": {
                "clients": reality_clients, "decryption": "none",
                # اگر ترافیک HTTP (XHTTP) بود، آن را به پورت داخلی بفرست
                "fallbacks": [{"dest": f"127.0.0.1:{XRAY_XH_INTERNAL_PORT}"}]
            },
            "streamSettings": {
                "network": "tcp", "security": "reality",
                "realitySettings": {"show": False, "dest": f"{REALITY_SNI}:443", "xver": 0, "serverNames": [REALITY_SNI], "privateKey": reality_keys["priv"], "shortIds": ["", "0123456789abcdef"]}
            }
        })
    
    cfg = {
        "log": {"loglevel": "warning", "access": XRAY_LOG}, "stats": {},
            ...
            if os.path.exists(XRAY_LOG):
                ...
                with open(XRAY_LOG, "r") as f:
                    ...
                    for ip, uid in matches:
                        ...
                        active_ips[uid][ip] = time.time(); user_last_active[uid] = time.time(); total_unique_users.add(uid)
        except: pass
...
def make_links(uid: str, domain: str, label: str) -> dict:
    ...
    if not REALITY_DOMAIN: 
        ...
        # هر دو از یک پورت و دامنه استفاده می‌کنند
    ...
    return {"ws": ws, "xhttp": xhttp, "reality": reality, "xhttp_reality": xhttp_reality}
