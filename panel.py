"""
پنل مدیریت XRAY — FastAPI + Nginx + Reality + Sub Page + Short ID
"""
...
def load_data():
    ...
    # اختصاص خودکار شناسه 7 رقمی به کاربران قدیمی
    updated = False
    for uid, info in LINKS.items():
        if "short_id" not in info:
            info["short_id"] = secrets.token_hex(4)[:7] # شناسه 7 رقمی
            updated = True
...
def make_links(uid: str, domain: str, label: str, sni: str, short_id: str) -> dict:
...
async def api_links(request: Request, token: Optional[str] = Cookie(None)):
    ...
    for uid, info in LINKS.items():
        ...
        out.append({
            ...
            "online_ips": online_count, "used_traffic": used_traffic, 
            "status": info.get("status", "active"),
            "data_limit": data_limit, "remaining_data": remaining_data,
            "remaining_days": remaining_days, "short_id": info.get("short_id", ""),
            **make_links(uid, domain, info["label"], info.get("sni", REALITY_SNI), info.get("short_id", ""))
        })
    return {"links": out}
...
async def create_link(request: Request, token: Optional[str] = Cookie(None)):
    ...
    info = {"label": label, "created_at": datetime.now().strftime("%Y-%m-%d %H:%M"), "sni": sni, "status": "active", "short_id": secrets.token_hex(4)[:7]}
    ...
    return {"ok": True, "uuid": uid, **make_links(uid, domain, label, sni, info["short_id"])}
...
async def subscription(sid: str, request: Request):
    ...
    if not user_info: return HTMLResponse("<h1>404 Not Found</h1><p>Subscription link is invalid.</p>", status_code=404)

    domain = get_domain(request)
    links = make_links(user_uid, domain, user_info["label"], user_info.get("sni", REALITY_SNI), sid)
    
    user_agent = request.headers.get("user-agent", "").lower()
    is_browser = any(b in user_agent for b in ["mozilla", "chrome", "safari", "opera", "edge", "firefox"])

    # اگر درخواست از سمت اپلیکیشن (v2rayNG) باشد، فایل Base64 می‌دهد
    if not is_browser:
        return PlainTextResponse(links["sub_base64"], media_type="text/plain")

    # اگر درخواست از مرورگر باشد، صفحه HTML می‌سازد
    ...
    html_content = f"""<!DOCTYPE html><html lang="fa" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>پنل کاربری {user_info['label']}</title><link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;500;600;700&display=swap" rel="stylesheet"><style>*{{box-sizing:border-box;margin:0;padding:0;font-family:'Vazirmatn',sans-serif}}body{{background:#f0f4ff;color:#1e293b;display:flex;justify-content:center;padding:20px}}.container{{max-width:600px;width:100%}}.header{{text-align:center;margin-bottom:30px}}.header h1{{color:#6366f1;font-size:24px;margin-bottom:5px}}.header p{{color:#64748b;font-size:14px}}.stats-grid{{display:grid;grid-template-columns:repeat(2,1fr);gap:15px;margin-bottom:30px}}.stat-card{{background:#fff;padding:20px;border-radius:16px;box-shadow:0 2px 8px rgba(0,0,0,0.05);text-align:center;border:1px solid #e2e8f0}}.stat-icon{{font-size:24px;margin-bottom:8px}}.stat-val{{font-size:18px;font-weight:700;color:#1e293b}}.stat-label{{font-size:12px;color:#64748b;margin-top:4px}}.config-box{{background:#fff;border-radius:12px;padding:15px;margin-bottom:12px;border:1px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center;gap:10px}}.config-info{{flex:1;overflow:hidden}}.config-title{{font-size:13px;font-weight:600;color:#6366f1;margin-bottom:4px}}.config-link{{font-size:10px;color:#94a3b8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;direction:ltr;text-align:left}}.copy-btn{{padding:8px 15px;background:#6366f1;color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:12px;font-weight:600;white-space:nowrap}}.copy-btn:hover{{background:#4f46e5}}.copy-all-btn{{display:block;width:100%;padding:15px;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;border:none;border-radius:12px;cursor:pointer;font-size:15px;font-weight:700;margin-top:20px}}.copy-all-btn:hover{{opacity:0.9}}.badge{{display:inline-block;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600;margin-bottom:15px}}.badge-active{{background:#d1fae5;color:#065f46}}.badge-expired{{background:#fee2e2;color:#991b1b}}</style></head><body><div class="container"><div class="header"><h1>⚡ پنل کاربری {user_info['label']}</h1><p>اطلاعات اشتراک و کانفیگ‌های شما</p><div class="badge {{'badge-active' if status=='active' else 'badge-expired'}}">{{{'🟢 فعال' if status=='active' else '🔴 منقضی شده'}}</div></div><div class="stats-grid"><div class="stat-card"><div class="stat-icon">📦</div><div class="stat-val">{fmt_bytes(used_traffic)}</div><div class="stat-label">حجم مصرف شده</div></div><div class="stat-card"><div class="stat-icon">📊</div><div class="stat-val">{fmt_bytes(remaining_data) if data_limit else 'نامحدود'}</div><div class="stat-label">حجم باقی‌مانده</div></div><div class="stat-card"><div class="stat-icon">📈</div><div class="stat-val">{fmt_bytes(data_limit) if data_limit else 'نامحدود'}</div><div class="stat-label">حجم کل</div></div><div class="stat-card"><div class="stat-icon">⏳</div><div class="stat-val">{remaining_days if expiry_time else 'نامحدود'}</div><div class="stat-label">روزهای باقی‌مانده</div></div></div><div id="configs"><div class="config-box"><div class="config-info"><div class="config-title">🔗 VLESS + WS + TLS</div><div class="config-link">{links['ws']}</div></div><button class="copy-btn" onclick="copyText('{links['ws']}', this)">کپی</button></div><div class="config-box"><div class="config-info"><div class="config-title">⚡ VLESS + XHTTP + TLS</div><div class="config-link">{links['xhttp']}</div></div><button class="copy-btn" onclick="copyText('{links['xhttp']}', this)">کپی</button></div><div class="config-box"><div class="config-info"><div class="config-title">🚀 VLESS + gRPC + TLS</div><div class="config-link">{links['grpc']}</div></div><button class="copy-btn" onclick="copyText('{links['grpc']}', this)">کپی</button></div><div class="config-box"><div class="config-info"><div class="config-title">🛡️ VLESS + HTTPUpgrade + TLS</div><div class="config-link">{links['httpupgrade']}</div></div><button class="copy-btn" onclick="copyText('{links['httpupgrade']}', this)">کپی</button></div><div class="config-box"><div class="config-info"><div class="config-title">👻 Trojan + WS + TLS</div><div class="config-link">{links['trojan']}</div></div><button class="copy-btn" onclick="copyText('{links['trojan']}', this)">کپی</button></div><div class="config-box"><div class="config-info"><div class="config-title">🌀 VMess + WS + TLS</div><div class="config-link">{links['vmess']}</div></div><button class="copy-btn" onclick="copyText('{links['vmess']}', this)">کپی</button></div><div class="config-box"><div class="config-info"><div class="config-title">🔥 VLESS + Reality + Vision</div><div class="config-link">{links['reality']}</div></div><button class="copy-btn" onclick="copyText('{links['reality']}', this)">کپی</button></div><div class="config-box"><div class="config-info"><div class="config-title">🛡️ VLESS + XHTTP + Reality</div><div class="config-link">{links['xhttp_reality']}</div></div><button class="copy-btn" onclick="copyText('{links['xhttp_reality']}', this)">کپی</button></div></div><button class="copy-all-btn" onclick="copyAll()">📋 کپی همه کانفیگ‌ها</button></div><script>function copyText(t,btn){{navigator.clipboard.writeText(t).then(()=>{{const o=btn.textContent;btn.textContent='کپی شد ✓';btn.style.background='#10b981';setTimeout(()=>{{btn.textContent=o;btn.style.background='#6366f1'}},2000)}})}}function copyAll(){{const t='{links['sub_base64']}';navigator.clipboard.writeText(atob(t)).then(()=>alert('همه کانفیگ‌ها کپی شدند ✓'))}}</script></body></html>"""
