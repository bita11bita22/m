"""
پنل مدیریت XRAY — FastAPI + Nginx + Reality + Sub Page + Clean IP + IP Stats
"""
...
async def stats_updater():
    ...
    while True:
        ...
        # خواندن لاگ Nginx برای ترافیک و شمارش کل ایپی‌ها
        try:
                    ...
                    for line in new_data.splitlines():
                        ...
                        if len(parts) == 2:
                            ...
                            if ip and ip != "127.0.0.1": 
                                ...
                                # برای کاربران Nginx، فقط می‌دانیم آنلاین هستند (عمدتاً 127.0.0.1 است پس نمی‌توانیم دقیق بشماریم)
                            stats["bytes"] += b
        except: pass

        # خواندن لاگ Xray برای شمارش آنلاین‌ها (Reality)
        try:
            if os.path.exists(XRAY_LOG):
                ...
                with open(XRAY_LOG, "r") as f:
                    ...
                    matches = re.findall(r'from\s+([0-9a-fA-F:.]+):\d+\s+accepted.*?email:\s*([a-f0-9-]{36})', new_data, re.IGNORECASE)
                    if not matches:
                        matches = re.findall(r'([0-9a-fA-F:.]+):\d+\s+accepted.*?email:\s*([a-f0-9-]{36})', new_data, re.IGNORECASE)
                    for ip, uid in matches:
                        if ip == "127.0.0.1":
                            if uid not in active_connections: active_connections[uid] = {}
                            active_connections[uid]["local"] = time.time() # آنلاین اما بدون تعداد دقیق
                        else:
                            if uid not in active_connections: active_connections[uid] = {}
...
async def api_stats(request: Request, token: Optional[str] = Cookie(None)):
    ...
    return {
        ...
        "total_connected": len(total_unique_ips), # کل ایپی‌های منحصر به فرد وصل شده
...
async def create_link(request: Request, token: Optional[str] = Cookie(None)):
    ...
    info = {"label": label, "created_at": datetime.now().strftime("%Y-%m-%d %H:%M"), "sni": sni, "status": "active", "short_id": secrets.token_hex(4)[:7], "clean_ip": clean_ip}
    ...
    return {"ok": True, "uuid": uid, **make_links(uid, domain, label, sni, info["short_id"], clean_ip)}
