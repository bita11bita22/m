...
def rate_limiter(ip: str, action: str, limit: int = 5, timeframe: int = 10):
    now = time.time()
    if len(RATE_LIMITS) > 200: RATE_LIMITS.clear()
...
def sync_xray_config():
    ...
    if warp_conf:
        inbounds.append({
            "port": XRAY_WARP_PORT, "listen": "127.0.0.1", "protocol": "vless", 
            "settings": {"clients": ws_xh_clients, "decryption": "none"}, 
            "streamSettings": {"network": "ws", "wsSettings": {"path": "/warp"}}, 
            "tag": "warp_in"
        })
        outbounds.append({
        ...
        routing_rules.append({"type": "field", "inboundTag": ["warp_in"], "outboundTag": "warp_out"})
            ...
            if os.path.exists(XRAY_LOG):
                ...
                for line in new_data.splitlines():
                    ...
                    try:
                        ...
                        if ip != "127.0.0.1" and len(total_unique_ips) < 2000: total_unique_ips.add(ip)
                    except: pass
        except: pass
...
def make_links(uid: str, domain: str, label: str, sni: str, short_id: str, clean_ip: str = "") -> dict:
    ...
    warp_link = f"vless://{uid}@{addr}:443?encryption=none&security=tls&type=ws&host={domain}&path=%2Fwarp&sni={domain}&fp=chrome#{label}-WARP"
        
    all_links = [ws, xhttp, grpc, httpupgrade, trojan, vmess, reality, xhttp_reality, warp_link]
    sub_link = f"https://{domain}/sub/{short_id}"
    sub_base64 = base64.b64encode("\n".join(all_links).encode()).decode()
    return {"ws": ws, "xhttp": xhttp, "grpc": grpc, "httpupgrade": httpupgrade, "trojan": trojan, "vmess": vmess, "reality": reality, "xhttp_reality": xhttp_reality, "warp": warp_link, "sub_link": sub_link, "sub_base64": sub_base64}
...
async def subscription(sid: str, request: Request):
    ...
    if not is_browser:
        ...
        all_links_list = [links['ws'], links['xhttp'], links['grpc'], links['httpupgrade'], links['trojan'], links['vmess'], links['reality'], links['xhttp_reality'], links['warp']]
    ...
    html_content = html_template.replace("__LABEL__", user_info['label']) \
        ...
        .replace("__LINK_WARP__", links['warp'])
