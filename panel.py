"""
پنل مدیریت XRAY — FastAPI + Nginx + Reality + Sub Page + Edit User + Restore (Final Fix)
"""
...
async function createUser(){const label=document.getElementById('new-label').value||'کاربر';const uuid=document.getElementById('new-uuid').value;const shortid=document.getElementById('new-shortid').value;const sni=document.getElementById('new-sni').value;const cleanip=document.getElementById('new-cleanip').value;const days=document.getElementById('new-days').value;const gb=document.getElementById('new-gb').value;const r=await fetch('/api/links',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label:label,uuid:uuid,short_id:shortid,sni:sni,days:days,gb:gb,clean_ip:cleanip})});const d=await r.json();closeAdd();document.getElementById('new-label').value='';document.getElementById('new-uuid').value='';document.getElementById('new-shortid').value='';document.getElementById('new-cleanip').value='';showLinks(d.uuid);loadUsers()}
function editUser(uid){var u=allUsers[uid];if(!u)return;document.getElementById('edit-uid').value=uid;document.getElementById('edit-label').value=u.label;document.getElementById('edit-days').value=0;document.getElementById('edit-gb').value=0;document.getElementById('edit-modal').classList.add('show')}
function closeEdit(){document.getElementById('edit-modal').classList.remove('show')}
async function saveEdit(){const uid=document.getElementById('edit-uid').value;const days=document.getElementById('edit-days').value;const gb=document.getElementById('edit-gb').value;const r=await fetch('/api/links/'+uid+'/edit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({days:days,gb:gb})});if(r.ok){closeEdit();loadUsers();alert('کاربر با موفقیت ویرایش شد ✓')}}
async function delUser(uid){if(!confirm('حذف این کاربر؟'))return;await fetch('/api/links/'+uid,{method:'DELETE'});loadUsers()}
...
function showQR(type){const link=document.getElementById('lnk-'+type).textContent;const img=document.getElementById('qr-img');img.src='https://api.qrserver.com/v1/create-qr-code/?size=250x250&data='+encodeURIComponent(link);document.getElementById('qr-modal').classList.add('show')}
function closeQR(){document.getElementById('qr-modal').classList.remove('show')}
async function changePass(){const r=await fetch('/api/change-password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({current:document.getElementById('cp-old').value,new:document.getElementById('cp-new').value})});const m=document.getElementById('cp-msg');if(r.ok){m.style.color='var(--green)';m.textContent='رمز با موفقیت تغییر کرد ✓'}else{m.style.color='var(--red)';m.textContent='رمز فعلی اشتباه است'}}
