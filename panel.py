"""
پنل مدیریت XRAY — Ultimate Edition + Security + UI Fix
"""
...
def rate_limiter(ip: str, action: str, limit: int = 5, timeframe: int = 10):
    ...
    if action not in RATE_LIMITS[ip]:
        RATE_LIMITS[ip][action] = []
    RATE_LIMITS[ip][action] = [t for t in RATE_LIMITS[ip][action] if now - t < timeframe]
    if len(RATE_LIMITS[ip][action]) >= limit:
        return False
...
async def subscription(sid: str, request: Request):
    ...
    if not is_browser:
        ...
        # هدر نمایش حجم و زمان در v2rayNG
