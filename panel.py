"""
پنل مدیریت XRAY — FastAPI + Nginx + Reality + Xray Stats API (Online Fixed)
"""
...
xray_process = None
xray_log_pos = 0
user_traffic = {}       # uid -> total_bytes_used
user_last_active = {}   # uid -> timestamp
active_ips = {}         # uid -> { ip: last_seen_time }
...
def generate_reality_keys():
    ...
    if not reality_keys["priv"]:
        try:
            ...
            if "Password (PublicKey):" in out:
                reality_keys["pub"] = out.split("Password (PublicKey):")[1].split("\n")[0].strip()
            elif "PublicKey:" in out:
                reality_keys["pub"] = out.split("PublicKey:")[1].split("\n")[0].strip()
            elif "Public key:" in out:
                reality_keys["pub"] = out.split("Public key:")[1].split("\n")[0].strip()
                
            if reality_keys["priv"] and reality_keys["pub"]:
                save_stats()
            else:
                log_err(f"Failed to parse keys from output: {out}")
        except Exception as e:
            log_err(f"Failed to generate Reality keys: {str(e)}")
        ...
        "inbounds": [
            {
                ...
                "settings": {"address": "127.0.0.1"}, "tag": "api_in"
            },
            *inbounds
        ],
...
async def stats_updater():
    ...
    while True:
        # ۱. خواندن ترافیک و تشخیص آنلاین بودن از API داخلی Xray
        try:
            ...
            if out:
                ...
                for stat in data.get("stat", []):
                    ...
                    if len(parts) == 4 and parts[0] == "user" and parts[2] == "traffic":
                        ...
                        # اگر کاربر ترافیک مصرف کرده بود، یعنی قطعا آنلاین است
                        if value > 0:
        ...
        # ۲. خواندن IPهای واقعی کاربران از لاگ Xray (برای شمارش تعداد دستگاه‌ها)
        try:
            if os.path.exists(XRAY_LOG):
                ...
                with open(XRAY_LOG, "r") as f:
                    ...
                    # پیدا کردن IP مبدا و UUID در خطوط لاگ
                    matches = re.findall(r'(\d+\.\d+\.\d+\.\d+):\d+ accepted.*?email:\s*([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})', new_data, re.IGNORECASE)
                    for ip, uid in matches:
                        if ip == "127.0.0.1": continue # نادیده گرفتن IP داخل کانتینر (Nginx)
        ...
        # ۳. پاکسازی کاربرانی که ۶۰ ثانیه است فعالیتی نداشته‌اند
        now = time.time()
        for uid in list(user_last_active.keys()):
            if now - user_last_active[uid] > 60:
                del user_last_active[uid]
                if uid in active_ips: del active_ips[uid]
        
        for uid in list(active_ips.keys()):
            for ip in list(active_ips[uid].keys()):
                if now - active_ips[uid][ip] > 60: del active_ips[uid][ip]
...
async def api_stats(request: Request, token: Optional[str] = Cookie(None)):
    ...
    return {
        ...
        "total_connected": len(total_unique_users), 
        "active_uuids": len(user_last_active), 
        "active_ips": len(user_last_active), 
        "bytes": stats["bytes"], 
        "uptime": uptime_str()
    }
...
async def api_links(request: Request, token: Optional[str] = Cookie(None)):
    ...
    for uid, info in LINKS.items():
        ...
        # اگر IP واقعی پیدا شد، تعداد آن را نشان بده، در غیر این صورت اگر آنلاین است عدد 1 را نشان بده
        real_ips = [ip for ip in active_ips.get(uid, {}) if ip != "127.0.0.1"]
        online_count = len(real_ips) if real_ips else (1 if is_online else 0)
        
        out.append({
            "uuid": uid, 
            "label": info["label"], 
            "created_at": info["created_at"], 
            "online_ips": online_count,
            "used_traffic": user_traffic.get(uid, 0),
            **make_links(uid, domain, info["label"])
        })
    return {"links": out}
