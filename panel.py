...
XRAY_API_PORT = 10085  # پورت API داخلی Xray
...
user_last_active = {}   # uid -> timestamp (for online status)
...
async def stats_updater():
    ...
    while True:
        # ۱. خواندن ترافیک و آنلاین بودن از API داخلی Xray
        try:
            ...
            if out:
                ...
                for stat in data.get("stat", []):
                    ...
                    # فرمت نام در Xray: user>>>UUID>>>traffic>>>uplink
                    if len(parts) == 4 and parts[0] == "user" and parts[2] == "traffic":
                        ...
                        # اگر کاربر ترافیک مصرف کرده بود، یعنی آنلاین است
                        if value > 0:
        ...
        # ۲. پاکسازی کاربرانی که ۳۰ ثانیه است فعالیتی نداشته‌اند (آفلاین کردن)
        now = time.time()
        for uid in list(user_last_active.keys()):
            if now - user_last_active[uid] > 30:
...
async def api_links(request: Request, token: Optional[str] = Cookie(None)):
    ...
    for uid, info in LINKS.items():
        out.append({
            ...
            "online_ips": 1 if uid in user_last_active else 0,
