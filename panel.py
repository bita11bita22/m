"""
پنل مدیریت XRAY — FastAPI + Nginx + Reality + Multi-Transport (Max Reality Guests)
"""
    ...
    if reality_keys["priv"]:
        inbounds.append({
            ...
            "settings": {
                ...
                "fallbacks": [
                    # مهمان‌های Reality (بر اساس Path مسیریابی می‌شوند)
                    {"path": "/xh", "dest": f"127.0.0.1:{XRAY_XH_INTERNAL_PORT}"},
                    {"path": "/ws", "dest": f"127.0.0.1:{XRAY_WS_PORT}"},
                    {"path": "/hu", "dest": f"127.0.0.1:{XRAY_HU_PORT}"},
                    {"path": "/tj", "dest": f"127.0.0.1:{XRAY_TJ_PORT}"},
                    # مهمان پیش‌فرض
...
def make_links(uid: str, domain: str, label: str) -> dict:
    ...
    if not REALITY_DOMAIN: 
        ...
        ws_reality = "خطا: REALITY_DOMAIN ست نشده"
        hu_reality = "خطا: REALITY_DOMAIN ست نشده"
        trojan_reality = "خطا: REALITY_DOMAIN ست نشده"
    elif not reality_keys["pub"]: 
        reality = "خطا: کلیدهای Reality ساخته نشدند"
        xhttp_reality = "خطا: کلیدهای Reality ساخته نشدند"
        ws_reality = "خطا: کلیدهای Reality ساخته نشدند"
        hu_reality = "خطا: کلیدهای Reality ساخته نشدند"
        trojan_reality = "خطا: کلیدهای Reality ساخته نشدند"
    else: 
        ...
        # مهمان‌های Reality (مسیرها با پورت TCP اصلی تطبیق می‌خورند)
        ws_reality = (f"vless://{uid}@{REALITY_DOMAIN}:{REALITY_PUBLIC_PORT}?encryption=none&security=reality&sni={REALITY_SNI}&fp=chrome&pbk={reality_keys['pub']}&sid=0123456789abcdef&type=ws&host={REALITY_SNI}&path=%2Fws#{label}-WS-Reality")
        hu_reality = (f"vless://{uid}@{REALITY_DOMAIN}:{REALITY_PUBLIC_PORT}?encryption=none&security=reality&sni={REALITY_SNI}&fp=chrome&pbk={reality_keys['pub']}&sid=0123456789abcdef&type=httpupgrade&host={REALITY_SNI}&path=%2Fhu#{label}-HU-Reality")
        trojan_reality = (f"trojan://{uid}@{REALITY_DOMAIN}:{REALITY_PUBLIC_PORT}?security=reality&sni={REALITY_SNI}&fp=chrome&pbk={reality_keys['pub']}&sid=0123456789abcdef&type=ws&host={REALITY_SNI}&path=%2Ftj#{label}-Trojan-Reality")
        
    return {"ws": ws, "xhttp": xhttp, "grpc": grpc, "httpupgrade": httpupgrade, "trojan": trojan, "vmess": vmess, "reality": reality, "xhttp_reality": xhttp_reality, "ws_reality": ws_reality, "hu_reality": hu_reality, "trojan_reality": trojan_reality}
